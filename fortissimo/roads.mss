...
#roads_gen::low[type='primary'][zoom>=9][zoom<12],
...
#road_label[type='motorway'][zoom>=13] {
  text-name: '[name]';
  [lang='fr'] {
    text-name: '[name].replace(" Président ", " Prés. ").replace(" Notre.[Dd]ame ", " ND. ").replace("Saint-", "St-").replace("Sainte-", "Ste-").replace("^Chemin", "Ch.").replace(" Maréchal ", " Mal. ").replace(" Général ", " Gén. ").replace("^Avenue", "Av.").replace("^Boulevard", "Bd.").replace("^Square","Sq.").replace("^Impasse","Imp.")';
  }
